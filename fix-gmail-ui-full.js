...
// 1. Add labels endpoint to gmail.js
let gmail = fs.readFileSync('server/routes/gmail.js', 'utf8');
if (!gmail.includes('/labels')) {
  gmail = gmail.replace(
    "// ── Attachment download ──",
    `// ── Gmail Labels ──
...
// ── Attachment download ──`
  ...
  console.log('  ✓ gmail.js — labels endpoint added');
}

// 2. Add API method
let api = fs.readFileSync('client/src/api.js', 'utf8');
if (!api.includes('getGmailLabels')) {
  api = api.replace(
    'gmailStatus:',
    "getGmailLabels: () => request('/gmail/labels'),\n  gmailStatus:"
  );
  fs.writeFileSync('client/src/api.js', api, 'utf8');
  console.log('  ✓ api.js — getGmailLabels method');
}

// 3. Rewrite PersonalInbox
const newInbox = `import React, { useState, useEffect, useRef } from 'react';
import { api } from '../api';
...
console.log('  ✓ PersonalInbox.jsx — full Gmail experience');
console.log('    • Compose button (Gmail-style blue pill)');
console.log('    • Primary/Social/Promotions/Updates tabs');
console.log('    • User custom labels in sidebar');
console.log('    • Unread counts per folder');
console.log('    • Compose modal (bottom-right, Gmail-style)');
console.log('    • Push to Queue only for supervisors/admins');
console.log('');
